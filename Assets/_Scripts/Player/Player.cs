...
public class Player : MonoBehaviour, IAgent, IHittable
{
    ...
    public int Health
    {
        ...
        set
        {
            health = Mathf.Clamp(value, 0, maxHealth); // health hiç geçemicek maxhealthi
    ...
    private void Awake()
    {
        playerWeapon = GetComponentInChildren<PlayerWeapon>(); // ammo için ulaşyoruz.
            ...
            if (resource != null)
            {
                switch (resource.ResourceData.ResourceType)
                {
                    case ResourceTypeEnum.Health:
                        ...
                        Health += resource.ResourceData.GetAmount(); // sağlık eklenen kısım
